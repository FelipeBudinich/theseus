...
The repository includes playable sample games, engine runtime modules, editor tooling, build tooling, documentation routes, and tests. It is designed so you can run games directly from source during development, edit levels in the browser, then bake production builds into `public/dist/`.
...
| `npm run bake` | Builds each game under `public/games/` with Vite and writes baked output to `public/dist/<game>/`. |
...
public/
  ...
  dist/                   Baked game list and per-game output generated by npm run bake
...
ESM level files store level data inside a module, register the level with the runtime registry, and export the level symbol and resource list. The editor's Node/Express API handles browsing files, saving `.js` and `.json` level files, and saving generated PNG images under `games/example/media/`.

## Assets

Source assets live under `public/games/example/media/` and are referenced with `games/example/media/...` paths from game code:

```js
const playerSheet = new ig.AnimationSheet('games/example/media/player.png', 75, 100);
const jumpSound = new ig.Sound('games/example/media/sounds/jump.*');
const font = new ig.Font('games/example/media/fredoka-one.font.png');
```

During source serving, images and audio are loaded from `/games/example/media/...`. During a baked build, Theseus can provide atlas manifests that let the runtime resolve the same `games/example/media/...` paths from generated assets in `/dist/<game>/assets/...`.
...
The font tool renders browser fonts into `ig.Font`-compatible PNG atlases. It supports preset character ranges, custom ranges, local font selection, manual CSS font families, fill color, spacing controls, alpha-threshold options, validation previews, and saving generated atlases into `public/games/example/media/`.

A generated font can be used like any other Theseus font asset:

```js
const font = new ig.Font('games/example/media/my-font.font.png');
font.firstChar = 32;
```
...
The bake command discovers immediate folders under `public/games/` that contain `index.html`, then runs one Vite build for each game. It writes the generated game list to:

```text
public/dist/index.html
```

Each game is baked into its own folder:
