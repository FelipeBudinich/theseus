...
ig.Game = ig.Class.extend({
	...
	getLevelByName: function( name ) {
		return ig.Game.getLevelByName(name);
	},
	...
	checkEntities: function() {
		...
		// A list of entities that the current one was already checked with,
...
ig.Game.levels = Object.create(null);

ig.Game.normalizeLevelName = function( name ) {
	if( !name ) {
		return '';
	}
	...
	return name
		.replace(/^Level/, '')
		.replace(/^([A-Z])/, function( _match, first ) { return first.toLowerCase(); });
};

ig.Game.registerLevel = function( name, levelData ) {
	var normalizedName = ig.Game.normalizeLevelName(name);

	if( !normalizedName || !levelData ) {
		return levelData || null;
	}

	ig.Game.levels[normalizedName] = levelData;
	return levelData;
};

ig.Game.getLevelByName = function( name ) {
	return ig.Game.levels[ig.Game.normalizeLevelName(name)] || null;
};
