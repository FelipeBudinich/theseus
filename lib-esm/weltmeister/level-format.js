...
const parseLevelSource = (source) => {
  ...
  const jsonSource = jsonMatch ? jsonMatch[1] : normalizedSource;
...
const serializeEsmLevel = ({
  ...
  const relativeImpactPath = toImportPath(normalizedFilePath, 'lib-esm/impact/impact.js');
  const relativeRegistryPath = toImportPath(normalizedFilePath, 'lib-esm/game/levels/registry.js');
  ...
  return [
    ...
    `import { registerLevel } from ${JSON.stringify(relativeRegistryPath)};`,
    '',
    `const ${levelSymbolName} = /*JSON[*/${jsonSource}/*]JSON*/;`,
    `const ${levelSymbolName}Resources = ${resourceSource};`,
    '',
    `registerLevel(${JSON.stringify(levelSymbolName)}, ${levelSymbolName});`,
    '',
